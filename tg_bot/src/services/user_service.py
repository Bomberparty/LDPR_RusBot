...
from src.domain.entities.user import User, Sources, Role
...
class UserService(IUserService):
    ...
    async def create_user(
        ...
        # Если ID пользователя есть в ADMIN_IDS из .env, присваиваем роль ADMIN
    ...
    async def validate_fio_part(self, part: str, part_name: str) -> str:
        ...
        if len(part) < 2:
            raise FioFormatError(f"{part_name} не может содержать менее 2 символов")
        if len(part) > 30:
            raise FioFormatError(f"{part_name} не может содержать более 30 символов")

        if ' ' in part:
            raise FioFormatError(f"{part_name} не может содержать пробелов.")

        if not re.match(r'^[А-Яа-яЁё\- ]+$', part):
            raise FioFormatError(f"{part_name} может содержать только русские буквы")

        if '--' in part:
            raise FioFormatError(f"{part_name} не может содержать несколько дефисов подряд")
        return part
